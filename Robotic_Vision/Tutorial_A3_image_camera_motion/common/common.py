...
def trnorm(T):
    n = T[0:3,0]; o = T[0:3,1]; a = T[0:3,2];
    n = np.cross(o, a);         # N = O x A
    o = np.cross(a, n);         # O = A x N
    Rn = np.stack((unit(n), unit(o), unit(a)), axis=1)
    Tn = np.hstack((Rn, T[0:3,3].reshape((3,1))))
    Tn = np.vstack((Tn, np.array([0, 0, 0, 1])))
    return Tn
